...
# All the columns are characters, apply(df, 2, class)
# set the class of columns 3,4,7:19 to numeric
for ( column in c(3,4,7:19) ){
  df[,column] <- as.numeric(df[,column])
  print(names(df)[column])
  print(summary(df[,column]))
}

# check whethere there are NA:
apply(df, 2, summary, rm.na=T)
