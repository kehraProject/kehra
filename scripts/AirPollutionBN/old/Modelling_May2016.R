...
df <- readRDS("~/kehra/data/EnglandDB_1981_2014.rds")
...
ind <- which(as.numeric(as.character(df$Year)) <= 2010)

training <- df[ind, ] # ~80% round(dim(training)[1]/dim(df)[1],2)
testing <- df[-ind, ] # ~20% round(dim(testing)[1]/dim(df)[1],2)
...
for (columnName in names(training)) {
  ...
  if (any(is.na(columnValues))) {
    ...
    # Calculate Expectation of missing value
    E <- predict(bn, node = columnName, current[is.na(columnValues),c(1:12)],
                 method = "bayes-lw")
    # E: replacing missing values with their (E)xpectation.
    current[is.na(columnValues), columnName] <- E
...
################################################################################
# LEARN THE BAYESIAN NETWORK USING PCALG #######################################
################################################################################
